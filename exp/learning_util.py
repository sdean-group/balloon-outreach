...
def plot_agent_summary(
    ...
    if objective == 'target':
        if not os.path.isdir('figs/modify_cost'):
            os.makedirs('figs/modify_cost')
        plt.savefig(os.path.join('figs/modify_cost', f'balloon_summary_{policy_name}_({target_pos[0]},{target_pos[1]})_({end_pos[0]},{end_pos[1]})_err_{distance:.2f}km.png'))

    else:
        if not os.path.isdir('figs/fly'):
            os.makedirs('figs/fly')
        plt.savefig(os.path.join('figs/fly', f'balloon_summary_{policy_name}_(({end_pos[0]},{end_pos[1]})_err_{distance:.2f}km.png'))
